...
\iattr{\_\_stereotype\_\_}{item basic stereotype}
\iattr{\_\_fixed\_stereotype\_\_}{item fixed stereotype}
...
\begin{description}
\item[basic stereotype] stereotype, which is always assigned
    to an item; for example interface item is annotated with
    \rstereotype{interface} stereotype; it is possible to annotate an item
    with other stereotypes if it has basic stereotype

\item[fixed stereotype] stereotype of an item, which cannot be changed;
    for example profile item can have only \rstereotype{profile}
    stereotype, metaclass item can have only \rstereotype{metaclass}
    stereotype; it is not possible to annotate an item with other
    stereotypes if there is fixed stereotype assigned
